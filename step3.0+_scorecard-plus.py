# step3_scorecard_plus.py
# 基于评分卡的高血脂风险预警模型（问题2） - 扩展版（含交互特征自动分箱）
...
OUTPUT_DIR = r"E:\MathorCup\Personal-Formal\results\risk_model\scorecard-plus"
...
def auto_bins_from_quantiles(series, n_bins=3):
    ...
    if series.nunique() < n_bins:
        ...
        boundaries = np.unique(boundaries)  # 去重
...
def run_scorecard(data_dir, output_dir):
    print("=" * 60)
    print("评分卡风险预警模型构建（扩展版 + 交互特征自动分箱）")
    print("=" * 60)
    ...
    # --- 2. 构建完整的 BINS 字典（手动 + 自动）---
    BINS = MANUAL_BINS.copy()
    for feat in INTERACTION_FEATS:
        # 使用训练集自动生成分箱边界
        bins = auto_bins_from_quantiles(X_train[feat], n_bins=3)
        BINS[feat] = bins
        print(f"\n特征 {feat} 自动分箱边界: {bins}")
